...
module.exports = function(req, res, next) {
    ...
    async.waterfall([
        ...
        // Check redirect uri
        function(cb) {
            req.oauth2.model.client.getRedirectUri(client, (err, uri) => {
                if (!uri)
        ...
        function(cb) {
            req.oauth2.model.client.checkGrantType(client, grantType, (err, correctType) => {
                if (!correctType)
        ...
        function(cb) {
            req.oauth2.model.client.transformScope(req.query.scope, (err, scopes) => {
                req.oauth2.model.client.checkScope(client, scopes, (err, clientScope) => {
                    if (!clientScope)
                        cb(new error.invalidScope('Invalid scope for the client'));
                    else {
                        scope = clientScope
                        req.oauth2.logger.debug('Scope check passed: ', clientScope);
                        cb();
                    }
                })
            })
        ...
        function(cb) {
            req.oauth2.model.user.fetchFromRequest(req, (err, foundUser) => {
                if (!foundUser)
                    cb(new error.invalidRequest('Failed to fetch logged user from request parameters'))
                else {
                    user = foundUser
                    req.oauth2.logger.debug('User fetched from request: ', foundUser);
                    cb();
                }
            })
        }
