...
module.exports = function(req, res, client, scope, user, redirectUri) {
    ...
    async.waterfall([
        ...
        // Issue new code
        function(cb) {
            req.oauth2.model.user.getId(user, (err, userId) => {
                req.oauth2.model.client.getId(client, (err, clientId) => {
                    req.oauth2.model.code.create(userId, clientId, scope, req.oauth2.model.code.ttl, function(err, data) {
                        if (err)
    ...
    function(err) {
        if (err) {
            response.error(req, res, err, redirectUri);
        }
